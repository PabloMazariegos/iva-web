  ...
  <div class="space-y-6">
    <div class="text-center">
      ...
      <p class="text-surface-600 dark:text-surface-400">
        Sube tu archivo Excel con las facturas de compras del período
      </p>
    </div>
      ...
      <template #content>
        <div v-if="!file" class="space-y-4">
          <FileUpload
            ...
            :auto="true"
            choose-label="Seleccionar Archivo de Compras"
            class="w-full"
            @select="onFileSelect"
          >
            <template #empty>
              <div class="flex flex-col items-center justify-center py-12">
                <Icon name="i-heroicons-shopping-cart" class="text-6xl text-surface-400 mb-4" />
        ...
        <div v-else class="space-y-4">
          <div class="flex items-center justify-between p-4 border border-surface-200 dark:border-surface-700 rounded-lg bg-blue-50 dark:bg-blue-900/20">
            <div class="flex items-center space-x-3">
              <Icon name="i-heroicons-document-check" class="text-2xl text-blue-600" />
              <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div class="text-center">
              <Icon name="i-heroicons-receipt-tax" class="text-lg text-surface-600 mb-1" />
              <p class="text-surface-500 dark:text-surface-400">Facturas de compras</p>
            </div>
            <div class="text-center">
    ...
    <div v-if="file" class="bg-surface-50 dark:bg-surface-900 p-4 rounded-lg">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
        ...
        <div class="flex items-center space-x-2">
          <Icon name="i-heroicons-building-office" class="text-surface-600" />
    ...
    <div v-if="file" class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
      <div class="flex items-start space-x-3">
        <Icon name="i-heroicons-exclamation-triangle" class="text-amber-600 mt-0.5" />
        <div>
          <h4 class="font-medium text-amber-800 dark:text-amber-200 mb-1">
            Importante para el Crédito Fiscal
          </h4>
          <p class="text-sm text-amber-700 dark:text-amber-300">
            Asegúrate de que las facturas de compras cumplan con los requisitos de la SAT para ser válidas como crédito fiscal.
          </p>
        </div>
      </div>
    </div>
